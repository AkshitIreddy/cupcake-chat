...
complete a chat. Record measured tokens/second and context settings. On 2026-09-05 the owner
profile's verified Qwen3 8B Q4_K_M completed two turns through managed b10679 CUDA 13 and measured
64.76 generated tokens/second in the 64-token app benchmark at 4,096 context. Streaming, Stop,
follow-up recovery, graceful VRAM-policy refusal, and unload were observed on provisional executable
`BF706D1F`. These results do not waive final package restart and interruption checks.
...
model load and restore `no` after unload and sidecar shutdown, including failure paths. Ordinary app
rendering does not take GPU-model ownership.
