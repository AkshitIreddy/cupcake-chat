...
BUILTIN_MODELS: tuple[ModelDescriptor, ...] = (
    _cloud(
        ...
        1_050_000,
        128_000,
        reasoning=(R.LOW, R.MEDIUM, R.HIGH, R.XHIGH, R.MAX),
        images=True,
        documents=True,
        cost=CostClass.HIGH,
        input_price="10",
    ...
    _cloud(
        ...
        500_000,
        None,
        reasoning=(R.LOW, R.MEDIUM, R.HIGH, R.XHIGH),
        images=True,
        input_price="2",
        output_price="6",
    ),
    ...
    _cloud(
        ...
        128_000,
        64_000,
        # The current Pydantic Cohere transport does not yet map Command A+
        # image parts or its reasoning controls, so those native model features
        # stay hidden until the actual packaged path can honor them.
    ),
)
...
class ModelCatalog:
    ...
    def select(self, model_id: str | None) -> ModelDescriptor:
        if not model_id:
            raise ValueError("model selection is required; Cupcake Chat never auto-routes")
