...
def test_nvidia_prefers_canonical_windows_system_executable(
    ...
    monkeypatch.setattr(hardware_module, "_nvidia_nvml", lambda: (None, None, None))
    monkeypatch.setenv("SYSTEMROOT", str(tmp_path))
    def missing_executable(_name: str) -> None:
        return None

    def completed_process(*_args: object, **_kwargs: object) -> SimpleNamespace:
        return SimpleNamespace(stdout="NVIDIA GeForce RTX 4080 Laptop GPU, 12282, 581.29\n")
    ...
    name, vram, driver = hardware_module._nvidia()
...
def test_nvidia_prefers_in_process_nvml_over_child_process(
    ...
    monkeypatch.setattr(
        hardware_module,
        "_nvidia_nvml",
        lambda: ("NVIDIA GeForce RTX 4090", 24.0, "610.43"),
    )
...
def test_hardware_profile_includes_device_identity_and_installed_packs(
    ...
    def nvidia() -> tuple[str, float, str]:
        return "NVIDIA GeForce RTX 4070", 12.0, "555.99"
